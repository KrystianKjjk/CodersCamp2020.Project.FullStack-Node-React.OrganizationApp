...
class TestProjectRepository extends ProjectRepository {
    ...
    async updateById(id: mongoose.Types.ObjectId, project: ProjectDBModel){
        const projectIndex = this.projects.findIndex(project => project._id === id);
...
describe("Project Service", () => {
    ...
    it("validates by using custom method", async () => {
        ...
        expect(service.validateProjectData(allDataNeededToCreate, true)).not.toBeNull();
        expect(service.validateProjectData(allDataNeededToCreate, false)).not.toBeNull();

        expect(service.validateProjectData(someDataNeededToUpdate, true)).toBeNull();
        expect(service.validateProjectData(someDataNeededToUpdate, false)).not.toBeNull();

        expect(service.validateProjectData(allDataButWrongTypes as any, true)).toBeNull();
        expect(service.validateProjectData(allDataButWrongTypes as any, false)).toBeNull();
    });
});