...
export const teamProjectSlice = createSlice({
  ...
  reducers: {
    ...
    switchSectionSelectionMode: (state) => {
      if (!state.loading || state.projectDeleteMode === true)
        state.sectionSelectionMode = !state.sectionSelectionMode
    },
  },
})
...
export function getProjectById(id: string) {
  return async (dispatch: Dispatch) => {
    ...
    try {
      ...
      try {
        const responseParentProject = await api.get(
          `/projects/${data.parentProjectId}`,
        )
        const dataParentProject = await responseParentProject.data
        data.referenceProjectName = dataParentProject.projectName

        try {
          const responseSection = await api.get(
            `/sections/${dataParentProject.sectionId}`,
          )
...
export function saveProjectSectionById(
  project: TeamProjectState,
  sectionId: string,
) {
  ...
  return async (dispatch: Dispatch) => {
    ...
    try {
      ...
      responseParentProjectName = (
        await api.get(`/projects/${newProject.parentProjectId}`)
      ).data.projectName
      responseSectionName = (
        await api.get(`/sections/${referenceProject.sectionId}`)
      ).data.name
    } catch (error) {
      dispatch(projectOperationFailure())
      console.log(error)
