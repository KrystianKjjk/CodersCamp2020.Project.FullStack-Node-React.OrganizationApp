...
export default class SheetService {
  ...
  getSheetInfo = async (sheet: GradeSheetData): Promise<GradeSheet> => {
    ...
    try {
      const projectRes = await this.api.get(
        '/teams/projects/' + sheet.projectID,
      )
  ...
  setMentorReviewerGrade = async (
    id: string,
    mentorId: string,
    grades: Grades,
  ) => {
    await this.api.put(`/grade/sheets/${id}/reviewers/${mentorId}/grades`, {
      grades,
    })
  }

  patchMentorReviewerGrade = async (
    id: string,
    mentorId: string,
    grades: Grades,
  ) => {
    await this.api.patch(`/grade/sheets/${id}/reviewers/${mentorId}/grades`, {
      grades,
    })
  }
