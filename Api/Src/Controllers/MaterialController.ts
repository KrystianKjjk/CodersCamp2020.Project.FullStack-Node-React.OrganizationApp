...
export default class MaterialController {
    ...
    getAllMaterials = async ( req: express.Request, res: express.Response, next?: express.NextFunction) => {
        const materials = await this.materialService.getAllMaterials();
        return res.status(200).json(materials);
    }

    getMaterialById = async ( req: express.Request, res: express.Response, next?: express.NextFunction) => {
        const material = await this.materialService.getMaterialById(req);
        return res.status(200).json(material);
    }

    updateMaterial = async ( req: express.Request, res: express.Response, next?: express.NextFunction) => {
        const material = await this.materialService.updateMaterial(req);
        return res.status(200).json(material);
    }

    deleteMaterial = async ( req: express.Request, res: express.Response, next?: express.NextFunction) => {
