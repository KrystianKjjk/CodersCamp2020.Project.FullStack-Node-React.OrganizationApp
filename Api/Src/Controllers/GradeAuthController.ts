...
export default class AuthGradeController {
    ...
    getPostUserGrades = async ( req: express.Request, res: express.Response, next: express.NextFunction ) => {
    ...
    patchDeleteUserGrades = async ( req: express.Request, res: express.Response, next: express.NextFunction ) => {
        ...
        if(userRole === UserType.Mentor) {
            ...
            return team.users.includes(user?._id) ? next() : res.status(403).json({message: 'FORBIDDEN'});
        }
        else next();
    }
}
