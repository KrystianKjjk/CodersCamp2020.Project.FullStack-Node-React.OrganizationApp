...
class TestCourseRepository extends CourseRepository {
    ...
    async updateById(id:mongoose.Types.ObjectId, course: Course & mongoose.Document<Course>){
        const courseIndex = this.courses.findIndex(course => course._id === id);
...
describe("CourseService", () => {
    ...
    test("should create course and fetch it", async()=>{
        const course = new CourseSchema({name: "CodersCamp 2021", sections:[], description: "description"});
    ...
    test("should fetch all courses", async ()=>{
        const course = new CourseSchema({name: "CodersCamp 2021", sections:[], description: "description"});
        const course2 = new CourseSchema({name: "CodersCamp 2022", sections:[], description: "description"});
