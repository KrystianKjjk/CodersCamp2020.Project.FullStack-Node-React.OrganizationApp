...
describe('Test AuthService ', () => {
    ...
    (async () => {
        const password = await service.hashPassword(user as User);
        let user2 = Object.assign({}, user);
        user2.password = password;
        await service.saveUser(user2 as User);
    })();

    test(`generate token`, () => {
        expect(typeof service.generateToken(user as User)).toBe('string');
    });
    test(`find user`, async () => {
        expect(await service.findUser(`test@test.pl`)).toBe(repo.users[0]);
    });
    test(`hash password`, async () => {
        expect(typeof await service.hashPassword(user as User)).toBe('string');
    });
